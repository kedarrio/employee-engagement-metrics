...
export const Button: React.FC<ButtonProps> = ({ 
  ...
  const baseStyles = "rounded-lg font-medium transition-all duration-200 flex items-center justify-center gap-2 active:scale-95 focus:outline-none focus:ring-2 focus:ring-offset-2";
  ...
  return (
    <button 
      type={type}
      className={`${baseStyles} ${sizeStyles[size]} ${variants[variant]} ${className} ${disabled || isLoading ? 'opacity-50 cursor-not-allowed active:scale-100' : ''}`}
